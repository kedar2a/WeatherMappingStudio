...
def from_csv(request):
	...
	with open('WeatherMappingStudio/WMI/CSV/'+fname)as csvfile:
	...
	csvfile.close()   
	print lat_list[1], lon_list[1], address_list[1]
	print 'Len : ', len(lat_list)	
...
@csrf_exempt
def through_overlay(request):
	location_obj = Location(latitude=Decimal(lat), longitude=Decimal(lon), address=add, sender_id=sender.number)
	location_obj.save()
